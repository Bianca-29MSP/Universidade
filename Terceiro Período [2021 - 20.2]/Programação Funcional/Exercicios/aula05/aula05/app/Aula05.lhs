...
-- terminal exemple: 
--   area (Rectangle (Point 9 3) 5 9)
--   area (Circle (Point 0 0) 15)
--   area (Triangle (Point 2 2) (Point 4 (-12)) (Point (-4) (-6)))

area :: Shape -> Float
area (Rectangle _ w h) = w * h
area (Circle _ r) = pi * (r*r)
area (Triangle a b c) = sqrt (p * (p-a') * (p-b') * (p-c')) --Fórmula de Heron
  where
    a' = dist(a, c)
    b' = dist(b, c) 
    c' = dist(a, b)
...
--Arvore usada:
arv = (INode 7 (INode 3 (INode 1 ILeaf ILeaf)
                        (INode 6 (INode 4 ILeaf ILeaf)
                                (INode 7 ILeaf ILeaf)))
              (INode 10 ILeaf
                      (INode 14 (INode 13 ILeaf ILeaf)
                              ILeaf)))

numElTree :: IntTree -> Int
numElTree ILeaf = 0
numElTree (INode x esq dir) = numElTree esq + 1 + numElTree dir

numLeafTree :: IntTree -> Int
numLeafTree ILeaf = 1 
numLeafTree (INode x esq dir) = numElTree esq + numElTree dir
\end{code}
...
{-
  ...
  Resolução do exercício:
-}
list = (ICons 6 (ICons 5 (ICons 4 (ICons 10 (ICons 11 (ICons 11 (ICons 2 INil)))))))
...
insertTree x (INode y esq dir)
  | x == y = INode y esq dir
  | x < y = INode y (insertTree x esq) dir
  | otherwise = INode y esq (insertTree x dir)
\end{code}
Exercício
=========
...
--sendOffers [(Customer "Ivete" "Sky" True), (Customer "Eve" "Morris" False)]

sendOffer :: Client -> Bool 
sendOffer (Customer _ _ v) = v

sendOffers :: [Client] -> [Client]
sendOffers [] = []
sendOffers cs = [c | c <- cs, sendOffer c == True]
\end{code}
