...
>   | sup < 0 || inf < 0 = []
>   | (sup - inf) >= (length x) = x
>   | otherwise = func' inf sup x 1
>       where
>           func' :: Int -> Int -> [Int] -> Int -> [Int] 
>           func' _ _ [] _ = []
>           func' inf sup (x:xs) i 
>               | (i >= inf && i <= sup) = x : func' inf sup xs (i + 1)
>               | otherwise = func' inf sup xs (i + 1)
...
> inRangeProperty inf sup [] = (length (inRange inf sup []) == 0)
> inRangeProperty inf sup x  
>   | inf < 0 || sup < 0 = lenInRange == 0
>   | inf == 0 && sup == 0 = lenInRange == 0
>   | inf > sup = lenInRange == 0
>   | interval >= lenList = lenInRange == lenList
>   | inf > lenList = lenInRange == 0
>   | inf == sup = lenInRange == 1
>   | sup > lenList = (lenInRange == (lenList - inf)) || (lenInRange == (lenList - inf + 1))
>   | otherwise = (interval == lenInRange) || (interval+1 == lenInRange)
>       where
>           interval = sup - inf
>           lenList = length x
>           inRange' = inRange inf sup x
>           lenInRange = length inRange'
