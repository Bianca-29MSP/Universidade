...
//              R.: As variáveis declaradas como static para ter o tempo de vida global, ou seja, permanecem alocados em memória guardando seu último valor mesmo após o encerramento de seu escopo e existindo em memória enquanto o programa estiver rodando,  isso para que haja uma incrementação no prefixo.
...
//              R.: Pois caso o contrário, sem a variável ter sido inicializada antes não há como utilizar.
 
//          (c) Por que e somado 1(uma) unidade ao tamanho do nome?
//              R.: Por causa do \0 que possui no final de toda string.

//          (d) Qual a vantagem obtida ao se declarar a variavel "cont" local
//              a rotina "criaNome" ao invez de declara-la global ao modulo?
//              R.: Pois isso permite que outra variável cout seja declarada em outro escopo do programa, além disso não há outra função que utilize essa variável. 
...
//              que acontece. 
//              R.: Ao deixar de alocar memória para str o programa dá erro de segmentação, pois acontece pelo mesmo motivo do sufixo.
...
// efeito.
// R.: Pois a cada execução é necessário encontrar 100kk de espaços livres na memória para armazenar as variáveis.
...
// R.: A maior vantagem é que variáveis com o mesmo nome podem ser criadas sem gerar 
//problemas ao longo do código, pois o nome de variável cout é bem comum, além disso outras 
//partes do código não usam o cout presente no escopo da classe com o mesmo propósito. 
//  Como é necessário ter controle da contagem durante toda execução do programa, ou seja, 
//uma variável global que armazena esse dado, a utilização do static se faz muito necessária.
...
// R.: Pois se trata de um método público e estático.
unsigned int Veiculo::cont = 0;
int main(int argc, char *argv[])
{
...
// sa�da.
//  R.: Foi declarado como virtual para que cada classe que tenha herança da classe 
//  Funcionario tenha que implementar esse método fazendo com que cada tipo de funcionário tenha o seu próprio reajuste de salário.
unsigned int Veiculo::cont = 0;
int main(int argc, char *argv[])
{
...
//     foi possivel.
//      R: Pois Veiculo é uma classe abstrata, ele não pode ser instaciada e sim
...
// R.: getClasse inicialmente não era um metodo constante, ele só poderia ser chamado 
//  por objetos não constantes. Para resolver este problema, basta definir o método como 
//  constante.
unsigned int Veiculo::cont = 0;
int main(int argc, char *argv[])
{
