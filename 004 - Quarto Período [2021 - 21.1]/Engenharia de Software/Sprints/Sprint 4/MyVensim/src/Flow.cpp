...
Flow::Flow(){
    this->input = NULL;
    this->output = NULL;
    this->equation.constant = 0;
    this->equation.PMax = 0;
    this->equation.observedSystem = "";
}
...
double Flow::getConstant() const
{
    return this->equation.constant; 
}

string Flow::getObservedSystem() const
{
    return this->equation.observedSystem; 
}

int Flow::getPMax() const
{
...
{
    System *s = NULL;
    if(!(getObservedSystem() == ""))
    {
        if(getInput() != NULL && getObservedSystem() == getInput()->getName()) s = getInput();
        else if(getOutput() != NULL && getObservedSystem() == getOutput()->getName())  s = getOutput();
    }
